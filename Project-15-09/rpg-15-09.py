...
def set_race(mode):
    ...
    if mode == 2:
        ...
        try:
            ...
            app.write("3. Uruk")
            app.write("4. Wizard")
            app.write("5. Haradrim") #New race, high attack, low defense and magic
            app.write("6. Dragon") #Kills everything pre much
            app.write("")
            app.wait_variable(app.inputVariable)
        ...
        try:
            ...
            app.write("4. Hobbit")
            app.write("5. Wizard")
            app.write("6. Dunedain") #High attack and defense, medium health and high magic capabilities
            app.write("7. Crusader - Only one worthy of facing the mighty foes in Ultra Nightmare!") #Made for ultra nightmare, high everything
            app.write("")
            app.wait_variable(app.inputVariable)
            race = app.inputVariable.get()
...
def create_player(mode, race, char_name):
    ...
    if mode == 2:
        if race == 1:
            ...
            player = character.Dragon(char_name, app)
        else:
...
def set_difficulty():  # Will Determine what enemies you will face, how much damage they will do and the AI's intelligence
    ...
    try:
        ...
        app.write("b - beginner") #Face weak enemy
        app.write("e - Easy")
        app.write("m - Medium")
        app.write("h - Hard")
        app.write("l - Legendary")
        app.write("un - Ultra Nightmare - Boss Mode")  # In honour of DOOM, will kill anyone but the crusader and dragon
        app.write("")
        app.wait_variable(app.inputVariable)
...
def create_enemies(mode, difficulty):
    ...
    if mode == 2:
        if difficulty == 'm':
            ...
            if roll == 0:
                enemies = [character.Ent("Treebeard", app)]
            elif roll == 1:
                enemies = [character.Skinchanger("Beorn", app)]
        elif difficulty == 'b':
...
def quit_game():  # Determines whether or not the game will end or you will play again, against another set of enemies
    """ Quits the game """
    try:
        app.write("Play Again? (y/n)") #WILL NOT CARRY OVER NEW STATS, levelup stats are only to help in battle
        app.write("")
        app.wait_variable(app.inputVariable)
...
def Mapper(): #Just keep moving, one input per second, will randomly generate it's own working map, if you break it, it regenerates another one with you at the beginning, so don't stuff around!
    name = char_name[0][0]
    table = []
    size = 25
    for i in range(size):
    ...
    yb = rolly
    table[yb][xb] = 'E'
    rollxc = random.randint(1, size-2)
    rollyc = random.randint(1, size-2)
    xc = rollxc
    yc = rollyc
    table[yc][xc] = 'E'
    ...
    while line:
        ...
        if line.strip() == 'd' or line.strip() == 'D':
            if x in range(xr-2, xr+2) and y in range(yr-1, yr+1):
                app.write("You fail to traverse the rocky terrain.")
                x -= 1
                y += 0
            else:
                x += 1

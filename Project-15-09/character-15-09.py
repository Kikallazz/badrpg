...
class Character:  # This is the character class, it dictates your stats
    ...
    def set_stance(self, stance_choice): #Added magic mods
    ...
    to be targeted). Returns True if target killed, False if still alive.'''
        ...
        if kill:
            ...
            blockroll = random.randint(1,10)
            if blockroll == 10:
                counterroll = random.randint(1,5)
                if counterroll == 5:
                    target.attack_enemy(self)
                    return None

    def defend_attack(self, att_damage): #These two have the counter attack in them, the defense roll was nerfed to balance the game
    ...
    a parameter. Returns True is character dies, False if still alive.'''
        ...
        if block_roll == 10:
            ...
            block = att_damage
            time.sleep(1)
            counter_attac = random.randint(1,5)
            if counter_attac == 5:
                self.app.write(self.name + " countered the attack!")
        ...
        if self.health <= 0:
            ...
            self.app.write("")
            self.exp += 100
            self.app.write(self.name + " has gained 100 experience points!")
            if self.exp >= 150*self.level:
              self.app.write(self.name + " has leveled up!")
              self.level += 1
              self.health += 25
              self.defense += 1
              self.attack += 2
    ...
    def cast_fireball(self, target):
        ...
        damage = int(roll * self.magic_mod * self.magic) - int(defense_roll * target.resistance)
        if damage < 0:
            damage = 0
    ...
    def cast_shield(self):
        ...
        time.sleep(1)
        if self.shield <= 25+self.magic*5:
            self.shield = 25+self.magic*5
    ...
    def cast_soul_steal(self, target): #IT WORKS, only the witch-king in ultra nightmare has it though
    ...
    def cast_heal(self): #pretty much a spell that does a health potion
        self.mana -= 20
    ...
    def use_manapotion(self): #New potion for peeps to get mo' mana
        """
        ...
        if self.shield > 0:
            ...
            while i <= 100:
                if i <= self.shield:
                    ...
                    shield_bar += " "
                i += 10
            shield_bar += "| " + str(self.shield) + " sp (" + str(int(self.shield * 100 / (25+self.magic*5))) + "%)"
...
class Dunedain(Character): #New class, rangers of the north!
...
class Dragon(Character): #High everything, will kill you in one hit, if it is lucky
...
class Nazgul(Character): #Deadly battle-wizard guy
    '''Defines the attributes of an Nazgul in the game. Inherits the constructor and methods
  of the Character class '''
    ...
    def move(self, player):
        """ Defines the AI for the Nazgul class """
        move_complete = Character.move(self, player)
        if self.mana >= 50:
            spellroll = random.randint(0, 1)
        if not move_complete:
            self.set_stance('a')
            if self.mana < 10 and player.mana > 0:
                self.cast_spell(3, player)

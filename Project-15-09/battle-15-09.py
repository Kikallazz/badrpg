...
"""
    ...
    def get_action(
            self):  # At the start of the turn, the player chooses their actions e.g. Aggressive, attack, (enemy 1)
    ...
    def select_spell(
            self):  # If player selects spells, it displays the available spells and then shows their resource cost and damage, etc.
        ...
        try:
            ...
            if player_race == "Dunedain":
                self.app.write("4. Heal wounds (20 mp)") #only for Dunedain, heals as much as a health potion
    ...
    def choose_stance(self):  # Determines your choices for attacking and defending, as well as taken dealt and taken
        try:
            ...
            self.app.write("")
            self.app.write("f - Flee From Battle") #flees from the game! :D
            self.app.write("")
            self.app.wait_variable(self.app.inputVariable)
            ...
            elif stance_choice == 'd': #ASCII looks cool, so I made some, based off of ones on the web
                self.app.write("")
                self.app.write("")
                self.app.write("")
                self.app.write("")
    ...
    def select_potions(self): #Potion select "menu"
        has_attacked = False
        player_race = self.player.__class__.__name__
        try:
    ...
    def do_player_actions(
            self):  # Tells player whether their attack hit the target and its damage, whether the they blocked, etc.
        ...
        while not self.player_won and not turn_over:
            ...
            if player_action == 3:
                potion_choice = self.select_potions() #enters you into the potion select screen then lets you choose, it then puts the potion into effect and then changes has_attacked to true, ending the turn
                if potion_choice == 2:
                    has_attacked = True
                elif potion_choice == 1:
                    has_attacked = True
    ...
    def do_enemy_actions(
            self):  # Determines whether or not the AI's attack hit you, whether they blocked your attack or if they healed using a potion, etc.
